...
================Loon==============
[Script]
cron "45 1,21 * * *" script-path=jd_speed_redpocke.js,tag=京东极速版红包
...
function reward_query() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_query", {
      linkId,
      "inviter": ["HXZ60he5XxF2LSrZg"][Math.floor((Math.random() * 1))]
    }), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
async function redPacket() {
  return new Promise(async resolve => {
    let body = {linkId, "inviter":["HXZ60he5XxG8XUF2LSrZg"][Math.floor((Math.random() * 1))]}
